...
function Weather() {
				...
				async position => {
					...
					try {
						void fetch(
							...
							.then(resp => {
								const city = (resp as CurrentCity).address.city.slice(6);
								setCurrentCity(city);
							});
					} catch (error) {
	...
	useEffect(() => {
		if (currentCity) {
			void fetch(`${api.base}weather?q=${currentCity}&units=metric&appid=${api.key}`)
				.then(async res => res.json())
				.then(result => {
					setDataWeather(result as PropsWeather);
				});
		}
	}, [currentCity]);

	const getResult = () => {
		void fetch(`${api.base}weather?q=${search}&units=metric&appid=${api.key}`)
			.then(async res => res.json())
			.then(result => {
				setDataWeather(result as PropsWeather);
			});
